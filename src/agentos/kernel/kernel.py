...
from agentos.kernel.config import AppConfig
...
class AgentOSKernel:
    ...
    def __init__(
        ...
        self._config = config
        self._write_gate = write_gate  # WriteGate from agentos.governance layer
        self._autonomy_policy = autonomy_policy  # AutonomyPolicy from agentos.policies layer
        ...
        from agentos.security.billing_fuse import BillingFuse
    ...
    async def wake_up(self, message: ChannelMessage) -> ChannelResponse:
        ...
        try:
            ...
            if any(keyword in text.lower() for keyword in ["validate", "verify", "write", "execute"]):
                from agentos.security.billing_fuse import TokenUsage
