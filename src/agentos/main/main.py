...
def create_app(bootstrap: ServiceBootstrap | None = None) -> FastAPI:
    ...
    from agentos.api.metrics import router as metrics_router
    from agentos.api.workflows import router as workflows_router
    from agentos.api.governance import router as governance_router
    ...
    async def health() -> dict[str, Any]:
        ...
        if b.kernel is None:
            status = "degraded"
        return {"status": status, "service": "agent-os", "version": "0.1.2", **details}
    ...
    dashboard_dir = Path("src/agentos/static/dashboard")
    dashboard_dir.mkdir(parents=True, exist_ok=True)

    # To support absolute paths like /_next generated by Next.js without basePath
    ...
    async def serve_dashboard(full_path: str = ""):
        """Serve SPA frontend and its static assets."""
        if full_path:
